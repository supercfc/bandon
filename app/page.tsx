...
export default function Home() {
  ...
  const itemSummary = useMemo(() => {
    const grouped = new Map<string, { product: string; price: number; quantity: number }>();
    for (const item of activeOrder?.items ?? []) {
      const product = item.product.trim();
      const key = `${product}\u0000${item.price}`;
      const current = grouped.get(key);
      if (current) current.quantity += 1;
      else grouped.set(key, { product, price: item.price, quantity: 1 });
    }
  ...
  async function copySummary() {
    ...
    } catch { setMessage("無法複製統計內容，請稍後再試"); }
  }
  ...
  return <main className="shell">
    ...
    {summaryOpen && activeOrder ? <div className="modal-backdrop" role="presentation"><section className="modal summary-modal" role="dialog" aria-modal="true" aria-label={`${activeOrder.title} 統計表`}><button className="close-modal" onClick={() => setSummaryOpen(false)} aria-label="關閉">×</button><p className="eyebrow">ORDER SUMMARY</p><h2>{activeOrder.title} 統計表</h2>{itemSummary.length ? <div className="summary-table-wrap"><table className="summary-table"><thead><tr><th>品項</th><th>單價</th><th>份數</th><th>小計</th></tr></thead><tbody>{itemSummary.map((item) => <tr key={`${item.product}-${item.price}`}><td>{item.product}</td><td>NT$ {item.price.toLocaleString("zh-TW")}</td><td>{item.quantity}</td><td>NT$ {(item.price * item.quantity).toLocaleString("zh-TW")}</td></tr>)}</tbody><tfoot><tr><th colSpan={2}>合計</th><td>{activeOrder.items.length} 份</td><td>NT$ {total.toLocaleString("zh-TW")}</td></tr></tfoot></table></div> : <p className="modal-help">目前還沒有訂購品項。</p>}<div className="summary-modal-actions"><button className="copy-summary" type="button" onClick={() => void copySummary()}>{summaryCopied ? "已複製統計內容" : "複製統計內容"}</button><button className="modal-submit" type="button" onClick={() => setSummaryOpen(false)}>返回訂單明細</button></div></section></div> : null}
  </main>;
}
